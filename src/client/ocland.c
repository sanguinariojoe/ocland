                                ...
                                cl_event *          event)
{
    ...
    // Send the object data, by the upload stream socket. Theoretically the
    // server has prepared the download stream to receive the object
    void *identifier = NULL;
    memcpy(&identifier, buffer->ptr_on_peer.object_ptr, sizeof(void*));
                               ...
                               cl_event *           event)
{
    ...
    socket_flag |= Send_size_t(sockfd, src_offset, MSG_MORE);
    socket_flag |= Send_size_t(sockfd, dst_offset, MSG_MORE);
    socket_flag |= Send_size_t(sockfd, cb, MSG_MORE);
    socket_flag |= Send(sockfd, &num_events_in_wait_list, sizeof(cl_uint), MSG_MORE);
    if(num_events_in_wait_list){
        for(i = 0; i < num_events_in_wait_list; i++) {
                              ...
                              cl_event *            event)
{
    ...
    socket_flag |= Send(sockfd, &comm, sizeof(unsigned int), MSG_MORE);
    socket_flag |= Send_pointer_wrapper(sockfd, PTR_TYPE_COMMAND_QUEUE, command_queue->ptr_on_peer, MSG_MORE);
    socket_flag |= Send_pointer_wrapper(sockfd, PTR_TYPE_MEM, src_image->ptr_on_peer, MSG_MORE);
    socket_flag |= Send_pointer_wrapper(sockfd, PTR_TYPE_MEM, dst_image->ptr_on_peer, MSG_MORE);
                                   ...
                                   cl_event *           event)
{
    ...
    socket_flag |= Send_size_t_array(sockfd, buffer_origin, 3, MSG_MORE);
    socket_flag |= Send_size_t_array(sockfd, region, 3, MSG_MORE);
    socket_flag |= Send_size_t(sockfd, buffer_row_pitch, MSG_MORE);
    socket_flag |= Send_size_t(sockfd, buffer_slice_pitch, MSG_MORE);
    socket_flag |= Send(sockfd, &num_events_in_wait_list, sizeof(cl_uint), MSG_MORE);
    for(i = 0; i < num_events_in_wait_list; i++) {
        socket_flag |= Send_pointer_wrapper(sockfd, PTR_TYPE_EVENT, event_wait_list[i]->ptr, MSG_MORE);
    }
    ...
    // So we must register a task for the download stream
    void *identifier = NULL;
    memcpy(&identifier, mem->ptr_on_peer.object_ptr, sizeof(void*));
    download_stream stream = getDataDownloadStream(command_queue->server);
    if(!stream){
        return CL_OUT_OF_RESOURCES;
    }
