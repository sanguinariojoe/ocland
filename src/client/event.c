...
/** @brief Function to be called when the server is reporting an event status
 * change.
                                      ...
                                      void*        user_data)
{
    ...
    if(!hasEvent(event)){
        VERBOSE("Unknown event (%p) received a status changing server signal\n",
                event);
        return;
    }

    if(info_size != sizeof(cl_int)){
        VERBOSE("Event (%p) received a status changing server signal with a wrong size (%lu bytes received, %lu expected)\n",
                event,
                info_size,
                sizeof(cl_int));
        return;
    }
    cl_int command_execution_status = *((cl_int*)info);

    cl_int flag = setEventStatus(event, command_execution_status);
    if(flag != CL_SUCCESS){
        VERBOSE("Event (%p) status changing failed (%s)\n",
                event,
                OpenCLError(flag));
        return;
    }

    // Try to release the event if it is completed (it will be retained if was
    // generated by the user)
    if(command_execution_status <= 0){
        cl_int flag = releaseEvent(event);
        if(flag != CL_SUCCESS){
            VERBOSE("Event (%p) releasing failed (%s)\n",
                    event,
                    OpenCLError(flag));
            return;
        }
...
/** @brief Register a task for the event in the server download stream.
 *
 * It will also call to retain the download stream, in order to allow the
 * context releasing before the event.
 * @param event Event which is requiring a task.
 * @return The registered task, NULL if errors are detected.
 */
task registerEventTask(cl_event event){
    // Get, or build up, the callbacks download data stream
    download_stream stream = createCallbackStream(event->server);
    if(!stream){
        return NULL;
    }
    ...
    if(!t){
        ...
        if(1 == stream_rcount) {
            event->server->callbacks_stream = NULL;
        }
        return NULL;
    }
    return t;
...
cl_int releaseEvent(cl_event  event)
{
    ...
    if(event->task_notify){
        flag = unregisterTask(event->server->callbacks_stream->tasks,
                              event->task_notify);
        if(flag != CL_SUCCESS){
            return CL_OUT_OF_HOST_MEMORY;
        }
    ...
    cl_uint stream_rcount = event->server->callbacks_stream->rcount;
    // Release the download stream
    flag = releaseDownloadStream(event->server->callbacks_stream);
    if(flag != CL_SUCCESS){
        return CL_OUT_OF_HOST_MEMORY;
    }
    if(1 == stream_rcount) {
