 ...
 *  Copyright (C) 2016  Timur Magomedov <tim239@yandex.ru>
 *
...
int main(int argc, char *argv[])
{
    ...
    for(i = 0; i < num_platforms; i++){
        ...
        for (j = 0; j < num_devices; j++)
        {
            ...
            //TODO: read the buffer several times together with clFlush and clFinish
            printf("\t\t\tReading buffer: ");
            flag = clEnqueueReadBuffer(queue, buf, CL_TRUE, 0, n * sizeof(cl_float), hbuf, 0, NULL, NULL);
            if(flag != CL_SUCCESS){
