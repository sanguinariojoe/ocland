...
int ocland_clEnqueueCopyBuffer(int* clientfd, validator v)
{
    ...
    // Receive the parameters
    socket_flag |= Recv_pointer(clientfd, PTR_TYPE_COMMAND_QUEUE, (void**)&command_queue);
    socket_flag |= Recv_pointer(clientfd, PTR_TYPE_MEM, (void**)&src_buffer);
    socket_flag |= Recv_pointer(clientfd, PTR_TYPE_MEM, (void**)&dst_buffer);
    socket_flag |= Recv_size_t(clientfd, &src_offset);
    socket_flag |= Recv_size_t(clientfd, &dst_offset);
    socket_flag |= Recv_size_t(clientfd, &cb);
    socket_flag |= Recv(clientfd, &num_events_in_wait_list, sizeof(cl_uint), MSG_WAITALL);
    if(num_events_in_wait_list){
        event_wait_list = (ocland_event*)malloc(
    ...
    // Call the OpenCL API
    cl_event e;
    flag = clEnqueueCopyBuffer(command_queue,
                               src_buffer,
                               dst_buffer,
                               src_offset,
                               dst_offset,
